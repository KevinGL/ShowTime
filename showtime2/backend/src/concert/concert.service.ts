...
export class ConcertService
{
    ...
    async createConcert(@Body() request)
    {
        //console.log(request);
        const newConcert = await new this.concertModel(request);

        if(request.user_connected)
        {
            if(request.user_admin)
            {
    ...
    {
        const existingConcert = await this.concertModel.findByIdAndUpdate(concertId, updateConcertDto, { new: true });
